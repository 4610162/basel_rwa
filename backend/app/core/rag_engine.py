...
7. 검색된 문서에 수식이 깨져 있거나 줄바꿈/기호가 손상된 경우,
   원문 의미를 유지하는 범위 내에서 LaTeX 문법에 맞게 정규화하여 다시 작성하세요.
8. 수식을 출력하기 전에 괄호 짝, \\left...\\right 짝, 분수/제곱근 문법이 올바른지 검증하세요.
9. LaTeX 문법이 확실하지 않은 경우 깨진 수식을 그대로 출력하지 말고,
   "원문 수식이 손상되어 정확한 LaTeX 복원이 어렵습니다."라고 답하세요.
10. JSON 문자열 안에서 백슬래시가 손실되지 않도록 수식은 항상 완성된 하나의 블록으로 작성하세요.
